    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3 text-sm font-noto">
          <span className="text-[#767676]">合计：{bookings.length}</span>
          <span className="text-[#767676]">·</span>
          <span className="text-green-700">已确认：{bookings.filter(b => b.status === "confirmed").length}</span>
          <span className="text-[#767676]">·</span>
          <span className="text-[#999999]">收入：${bookings.filter(b => b.status !== "cancelled").reduce((s, b) => s + b.amount, 0)}</span>
      ...
      {sorted.length === 0 ? (
        <div className="bg-white border border-[#E5E5E5] rounded-sm p-16 text-center">
          <p className="font-noto text-[#767676] text-xl mb-2">暂无预约记录</p>
          <p className="font-noto text-[#767676] text-sm">访客预约后将在此显示。</p>
        </div>
      ) : (
        <div className="space-y-3">
          {sorted.map((b) => {
            ...
            return (
              <div key={b.id} className="bg-white border border-[#E5E5E5] rounded-sm p-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {preSurvey && (
                      <div className="mt-3">
                        <p className="text-xs text-[#767676] font-noto mb-1.5">兴趣方向</p>
                        ...
                        {preSurvey.openQuestion && (
                          <p className="text-sm font-noto text-[#666666] mt-2 border-t border-[#E5E5E5] pt-2">
                            「{preSurvey.openQuestion}」
                          </p>
                        )}
                      </div>
                  ...
                  <div className="text-right flex-shrink-0 flex flex-col items-end gap-1">
                    <p className="font-noto text-2xl text-[#E51B23] font-light">${b.amount}</p>
                    <p className="text-xs text-[#767676] font-noto">{PAYMENT_LABELS[b.paymentType] ?? b.paymentType}</p>
                    {b.tourDate && (
                      <p className="text-sm font-noto text-[#999999]">
