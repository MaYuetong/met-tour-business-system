...
export default function GuidesPage() {
  ...
  if (loading) return <p className="font-noto text-[#767676]">加载中…</p>;
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-noto text-sm text-[#767676] mt-1">
            管理讲解员信息及可兼职日期。预约日期匹配时自动分配对应讲解员，无匹配则默认分配第一位在岗讲解员。
          </p>
        </div>
        <button onClick={() => setShowAdd((v) => !v)}
          className="bg-[#E51B23] text-white px-5 py-2.5 font-noto text-sm hover:bg-[#C01018] transition-colors flex-shrink-0">
          + 添加讲解员
        </button>
      </div>
      ...
      {/* Add guide form */}
      {showAdd && (
        <div className="bg-white border border-[#E51B23]/30 p-6 mb-6">
          <h3 className="font-noto text-[#1A1A1A] mb-5">新讲解员信息</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
            <div>
              <label className="block font-noto text-xs text-[#767676] mb-2">姓名 *</label>
              <input value={newGuide.name}
                onChange={(e) => setNewGuide((p) => ({ ...p, name: e.target.value }))}
                placeholder="例如：小明"
                className="w-full border border-[#E5E5E5] px-3 py-2.5 font-noto text-sm text-[#1A1A1A] focus:outline-none focus:border-[#E51B23] transition-colors" />
            </div>
            <div>
              <label className="block font-noto text-xs text-[#767676] mb-2">微信号 *</label>
            ...
            <div>
              <label className="block font-noto text-xs text-[#767676] mb-2">邮箱（选填）</label>
          ...
          <div className="flex gap-3">
            <button onClick={handleAddGuide} disabled={saving || !newGuide.name || !newGuide.wechatId}
              className="bg-[#E51B23] text-white px-6 py-2 font-noto text-sm hover:bg-[#C01018] transition-colors disabled:opacity-50">
              {saving ? "保存中…" : "保存"}
            </button>
            <button onClick={() => { setShowAdd(false); setNewGuide({ name: "", wechatId: "", email: "" }); }}
              className="border border-[#E5E5E5] px-6 py-2 font-noto text-sm text-[#767676] hover:border-[#E51B23] transition-colors">
              取消
            </button>
          </div>
        </div>
      ...
      {guides.length === 0 ? (
        <div className="bg-white border border-[#E5E5E5] p-8 text-center">
          <p className="font-noto text-[#767676]">暂无讲解员。</p>
        </div>
      ) : (
        <div className="space-y-4">
          {guides.map((guide) => (
            <div key={guide.id} className="bg-white border border-[#E5E5E5]">
              {/* Guide row */}
              <div className="flex items-center gap-4 p-5">
                <div className="w-10 h-10 bg-[#E51B23] flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-noto text-sm text-[#767676]">
                    微信：{guide.wechatId}
                    {guide.email && <span> · {guide.email}</span>}
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                  <span className={`text-xs px-2 py-1 font-noto border ${guide.isActive ? "bg-green-50 text-green-700 border-green-200" : "bg-[#F5F5F5] text-[#767676] border-[#E5E5E5]"}`}>
                    {guide.isActive ? "在岗" : "暂停"}
                  </span>
                  <span className="text-xs font-noto text-[#767676] border border-[#E5E5E5] px-2 py-1">
                    {guide.availableDates.length} 个日期
                  </span>
                  <button onClick={() => setExpandedId(expandedId === guide.id ? null : guide.id)}
                    className="font-noto text-xs text-[#E51B23] border border-[#E51B23]/30 px-3 py-1 hover:bg-[#E51B23]/5 transition-colors">
                    {expandedId === guide.id ? "收起 ↑" : "管理日期 ↓"}
                  </button>
                  <button onClick={() => patch(guide.id, { isActive: !guide.isActive })}
                    className="font-noto text-xs text-[#767676] border border-[#E5E5E5] px-3 py-1 hover:border-[#E51B23] hover:text-[#E51B23] transition-colors">
              ...
              {/* Expanded date panel */}
              {expandedId === guide.id && (
                <div className="border-t border-[#E5E5E5] p-5 bg-[#F5F5F5]">
                  <p className="font-noto text-xs text-[#767676] mb-4">
                    设置可兼职日期。预约日期与此列表匹配时，客户邮件将显示该讲解员的微信号。
                    未设置日期的讲解员将作为默认兜底。
                  </p>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="overflow-hidden border border-[#E5E5E5] bg-white focus-within:border-[#E51B23] transition-colors">
                      <input type="date"
                    ...
                    <button onClick={() => addDate(guide)} disabled={!dateInputs[guide.id]}
                      className="bg-[#E51B23] text-white px-4 py-2.5 font-noto text-xs hover:bg-[#C01018] transition-colors disabled:opacity-40">
                      添加日期
                    </button>
                  </div>
                  {guide.availableDates.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {guide.availableDates.map((d) => (
                        <div key={d} className="flex items-center gap-1.5 bg-white border border-[#E5E5E5] px-3 py-1.5">
                          <span className="font-noto text-sm text-[#1A1A1A]">{d}</span>
                          <button onClick={() => removeDate(guide, d)}
                            className="text-[#767676] hover:text-[#E51B23] transition-colors text-xl leading-none ml-1">
                            ×
                          </button>
                        </div>
                      ))}
      ...
      <div className="mt-8 bg-[#F5F5F5] border border-[#E5E5E5] p-5">
        <p className="font-noto text-xs text-[#767676] leading-relaxed">
