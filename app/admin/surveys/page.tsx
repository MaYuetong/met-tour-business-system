  ...
  return (
    <div className="space-y-12">
      ...
      <section>
        ...
        {preSorted.length === 0 ? (
          <div className="bg-white border border-[#E0D5C8] p-10 text-center">
            <p className="font-noto text-[#8B7D72]">暂无参观前问卷数据</p>
          </div>
        ) : (
              ...
              <div key={r.id} className="bg-white border border-[#E0D5C8] p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-[#8B7D72] font-noto mt-0.5">
                ...
                <div className="grid sm:grid-cols-3 gap-3 text-sm">
                  <div className="bg-[#F8F5F0] px-3 py-2">
                    <p className="font-sans-ui text-[10px] tracking-wider text-[#8B7D72] uppercase mb-1">首次参观</p>
                    <p className="font-noto text-[#1A1A1A]">{r.firstVisit === "yes" ? "是" : "否"}</p>
                  </div>
                  <div className="bg-[#F8F5F0] px-3 py-2">
                    <p className="font-sans-ui text-[10px] tracking-wider text-[#8B7D72] uppercase mb-1">知识背景</p>
                    <p className="font-noto text-[#1A1A1A]">{KNOWLEDGE[r.knowledgeLevel] ?? r.knowledgeLevel}</p>
                  </div>
                  <div className="bg-[#F8F5F0] px-3 py-2">
                    <p className="font-sans-ui text-[10px] tracking-wider text-[#8B7D72] uppercase mb-1">偏好体验</p>
                ...
                {r.interests?.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {r.interests.map((i) => (
                      <span key={i} className="text-xs bg-white border border-[#E0D5C8] text-[#6B5E52] px-2 py-0.5 font-noto">{i}</span>
                    ))}
                  </div>
                )}
                {r.openQuestion && (
                  <p className="mt-3 font-noto text-sm text-[#6B5E52] border-l-2 border-[#C9A84C] pl-3 leading-relaxed">
                    「{r.openQuestion}」
                  </p>
                )}
              </div>
      ...
      <section>
        ...
        {postSorted.length === 0 ? (
          <div className="bg-white border border-[#E0D5C8] p-10 text-center">
            <p className="font-noto text-[#8B7D72]">暂无参观后问卷数据</p>
          </div>
        ) : (
          <div className="space-y-3">
            {postSorted.map((r) => (
              <div key={r.id} className="bg-white border border-[#E0D5C8] p-5">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-0.5">
                      <p className="font-noto text-[#A6192E] text-2xl font-light">{r.ratings.overall}/5</p>
                ...
                {r.testimonial && (
                  <p className="font-noto text-sm text-[#6B5E52] border-l-2 border-[#A6192E] pl-3 leading-relaxed mb-2">
                    「{r.testimonial}」
                  </p>
                )}
                {r.improvement && (
                  <p className="font-noto text-xs text-[#8B7D72] mt-2">改进建议：{r.improvement}</p>
                )}
              </div>
            ))}
          </div>
