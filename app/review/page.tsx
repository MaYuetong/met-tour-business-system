...
function StarPicker({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  ...
  return (
    <div>
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button key={star} type="button" onClick={() => onChange(star)}
            ...
            <svg width="40" height="40" viewBox="0 0 24 24"
              fill={active >= star ? "#E51B23" : "none"}
              stroke={active >= star ? "#E51B23" : "#E5E5E5"}
              strokeWidth="1.5">
      ...
      {active > 0 && <p className="text-sm font-noto text-[#E51B23] h-5">{labels[active]}</p>}
    </div>
  );
}
      ...
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-6 py-16">
          <div className="text-center max-w-md">
            <div className="w-14 h-14 bg-[#E51B23] flex items-center justify-center mx-auto mb-8">
            ...
            <Link href="/"
              className="inline-block bg-[#E51B23] text-white px-10 py-4 font-sans-ui text-xs tracking-widest uppercase hover:bg-[#C01018] transition-colors">
              返回首页
            </Link>
          </div>
        </main>
              ...
              {SECTIONS.map((s) => (
                <button key={s.id} type="button" onClick={() => setField("section", s.id)}
                  className={`py-4 px-5 border rounded-sm text-left font-noto text-sm transition-all duration-200 ${
                    form.section === s.id ? "border-[#E51B23] bg-white text-[#E51B23]" : "border-[#E5E5E5] bg-white text-[#1A1A1A] hover:border-[#E51B23]/50"
                  }`}>
                  {s.label}
                </button>
              ))}
              ...
              {[{ id: "yes", label: "强烈推荐" }, { id: "no", label: "不确定" }].map((opt) => (
                <button key={opt.id} type="button" onClick={() => setField("wouldRecommend", opt.id)}
                  className={`py-5 px-6 border rounded-sm font-noto transition-all duration-200 ${
                    form.wouldRecommend === opt.id ? "border-[#E51B23] bg-[#E51B23] text-white" : "border-[#E5E5E5] bg-white text-[#1A1A1A] hover:border-[#E51B23]"
                  }`}>
                  {opt.label}
                </button>
              ))}
                ...
                <button key={opt.id} type="button" onClick={() => setField("allowQuote", opt.id)}
                  className={`py-4 px-5 border rounded-sm font-noto text-sm transition-all duration-200 ${
                    form.allowQuote === opt.id
                      ? opt.id === "yes" ? "border-green-600 bg-green-600 text-white" : "border-[#767676] bg-[#767676] text-white"
                      : "border-[#E5E5E5] bg-[#F5F5F5] text-[#1A1A1A] hover:border-[#767676]"
                  }`}>
                  {opt.label}
                </button>
              ))}
